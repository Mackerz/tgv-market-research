from sqlalchemy import Column, Integer, String, DateTime, Boolean, Text, ForeignKey, BigInteger, JSON
...
class ReportSettings(Base):
    ...
    survey_id = Column(BigInteger, ForeignKey("surveys.id"), unique=True, nullable=False)

    # Age ranges stored as JSON array of objects: [{"min": 0, "max": 18, "label": "0-18"}, ...]
    age_ranges = Column(JSON, nullable=False, default=[
...
class QuestionDisplayName(Base):
    ...
    id = Column(BigInteger, primary_key=True, index=True)
    report_settings_id = Column(BigInteger, ForeignKey("report_settings.id"), nullable=False)
